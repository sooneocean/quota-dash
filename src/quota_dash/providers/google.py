...
class GoogleProvider(Provider):
    ...
    async def get_proxy_data(self) -> ProxyData | None:
        ...
        return await query_provider_data(self._db_path, "google")
    ...
    async def get_context_window(self) -> ContextInfo:
        ...
        if proxy is not None and proxy.input_tokens > 0:
            return ContextInfo(
                ...
                max_tokens=1048576,
                percent_used=proxy.input_tokens / 1048576 * 100,
                model=proxy.model or "gemini-2.0-flash",
                note="last call snapshot",
            )
        return ContextInfo(
            used_tokens=0, max_tokens=1048576,
            percent_used=0.0, model="gemini-2.0-flash",
            note="approximation",
        )
